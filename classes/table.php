...
abstract class tablePts {
            ...
            if($id) {
                ...
                foreach($data as $field) {
                    ...
                    foreach($field as $k => $v) {
                        if(isset($this->_fields[$k])) {
                            $row[$k] = toeCreateObjPts('fieldPts', array(
                                    $this->_fields[$k]->name,
                                    $this->_fields[$k]->html,
                                    $this->_fields[$k]->type,
    ...
    public function get($fields = '*', $where = '', $tables = '', $return = 'all') {
        ...
        return dbPts::get($query, $return);
    }
    public function store($data, $method = 'INSERT', $where = '') {
		$this->_clearErrors();
        $method = strtoupper($method);
		if($this->_escape) {
			$data = dbPts::escape($data);
		}
        $query = '';
        switch($method) {
            case 'INSERT':
        ...
        if(empty($fields)) {
            $this->_addError(__('Nothig to update', PTS_LANG_CODE));
            return false;
        }
        ...
        if(dbPts::query($query)) {
			...
			$this->_addError(PTS_TEST_MODE ? dbPts::getError() : __('Database error. Please contact your developer.', PTS_LANG_CODE));
        return false;
    }
    ...
    /**
     * Add new fieldPtsPts for children table (@see class field)
    ...
    protected function _addField($name, $html = 'text', $type = 'other', $default = '', $label = '', $maxlen = 0, $dbAdapt = '', $htmlAdapt = '', $description = '') {
        $this->_fields[$name] = toeCreateObjPts('fieldPts', array($name, $html, $type, $default, $label, $maxlen, $dbAdapt, $htmlAdapt, $description));
        return $this;
    }
	/**
    ...
    public function exists($value, $field = '') {
        if(!$field)
            $field = $this->_id;
        return dbPts::get('SELECT '. $this->_id. ' FROM '. $this->_table. ' WHERE '. $field. ' = "'. $value. '"', 'one');
    }
    ...
    public function getLastInsertID() {
        return dbPts::get('SELECT MAX('. $this->_id. ') FROM '. $this->_table, 'one');
    }

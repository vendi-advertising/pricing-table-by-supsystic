...
class htmlPts {
    ...
    static public function ajaxfile($name, $params = array('url' => '', 'value' => '', 'fid' => '', 'buttonName' => '')) {
        ...
        $out = '';
        if(strpos($params['url'], 'pl='. PTS_CODE) === false)
			$params['url'] = uriPts::_(array('baseUrl' => $params['url'], 'pl' => PTS_CODE));
        ...
        $out .= '<script type="text/javascript">// <!--
                jQuery(document).ready(function(){
                    new AjaxUpload("#toeUploadbut_'.$name.'", { 
                        ...
                        (empty($params['onPtsmit']) ? '' : ',  onPtsmit: '. $params['onPtsmit']. '').
    ...
    static protected function _loadProductsOptions() {
        if(empty(self::$productsOptions)) {
            $products = framePts::_()->getModule('products')->getModel()->get(array('getFields' => 'post.ID, post.post_title'));
            if(!empty($products)) {
	...
	static public function fontsList($name, $params = array('value' => '')) {
		...
		if(isset($params['default'])) {
			$params['options'] = array(PTS_DEFAULT => $params['default']);
		}
	...
	/*static public function radialProgress($progress, $attrs = array()) {
		framePts::_()->addStyle('radial-progress', PTS_CSS_PATH. 'radial-progress.css');

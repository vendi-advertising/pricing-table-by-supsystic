...
abstract class viewPts extends baseObjectPts {
	...
	public function getPath($tpl) {
		...
		$plTemplate = framePts::_()->getModule('options')->get('template');		// Current plugin template
		if(empty($plTemplate) || !framePts::_()->getModule($plTemplate))
			$plTemplate = '';
	...
	public function getModel($code = '') {
		return framePts::_()->getModule( $this->_code )->getController()->getModel($code);
	}

...
class fieldPts {
	 ...
	 */
	public $adapt = array('htmlPts' => '', 'dbFrom' => '', 'dbTo' => '');
	/**
	...
	public function setValue($value, $fromDB = false) {
		if(isset($this->adapt['dbFrom']) && $this->adapt['dbFrom'] && $fromDB)
			$value = fieldAdapterPts::_($value, $this->adapt['dbFrom'], fieldAdapterPts::DB);
		$this->value = $value;
	}
	...
	public function drawHtml($tag, $id) {
		if(method_exists('htmlPts', $this->html)) {
			...
			if ($this->name == 'default_value') {
				$optionsFromDb = framePts::_()->getModule('optionsPts')->getHelper()->getOptions($id);
				if (!empty($optionsFromDb)) {
					$options = array(0 => __('Select', PTS_LANG_CODE));
			...
			if(!empty($this->errorEl)) {
				if(is_bool($this->errorEl))
					$errorEl = '<div class="toeErrorForField toe_'. htmlPts::nameToClassId($this->name). '"></div>';
	...
	public function viewField($name, $fieldValue = '') {
		$method = $this->html;
		$options = framePts::_()->getModule('optionsPts')->getHelper()->getOptions($this->id);
		$attrs = '';
		...
		$params = array('optionsPts'=>$options, 'attrs' => $attrs, 'value' => $fieldValue, 'checked' => $checked);
		$output = '';
		if(method_exists('htmlPts', $method)) {
			$output .= htmlPts::$method($name, $params);
			$output .= htmlPts::hidden('extra_field['.$this->name.']',array('value'=>$this->id));
		}
		return $output;
	}
		...
		if ($this->name == "params") {
			...
			if ($tag > 0 || $id == 0) {
				...
				$output .= '<div class="options image_tag"'.$image_tag.'>'.__('Dimensions', PTS_LANG_CODE).':<br />';
					...
					$output .= htmlPts::text('params[width]',array('value'=>$width)).'<br />';
					$output .= __('height', PTS_LANG_CODE).':<br />';
					$output .= htmlPts::text('params[height]',array('value'=>$height)).'<br />';
				$output .= '</div>';
			}
	...
	public function drawConfig($module, $additionalOptions = array()) {
		if(!framePts::_()->getModule($module)) 
			return false; 
		// check for xml file with params structure  
	   if(framePts::_()->getModule($module)->isExternal())
		   $config_xml = framePts::_()->getModule($module)->getModDir(). 'mod.xml';
	   else
		   $config_xml = PTS_MODULES_DIR.$module.DS.'mod.xml';
		   ...
		   foreach ($params as $param) {
			   ...
			   foreach ($configOptions as $key=>$value){
				  ...
				  // filling the parameters to build html element
					 $htmlParams = array('value'=>$fieldValue,'optionsPts'=>$options);
					 if($method == 'checkbox') {
						 $htmlParams['value'] = 1;

 ...
 */
if(!function_exists('dateToTimestampPts')) {
    function dateToTimestampPts($date) {
        if(empty($a))
            return false;
        $a = explode(PTS_DATE_DL, $date);
...
if(!function_exists('importPts')) {
    function importPts($path) {
        if(file_exists($path)) {
            require($path);
            return true;
        }
...
if(!function_exists('importClassPts')) {
    function importClassPts($class, $path = '') {
        if(!class_exists($class)) {
            if(!$path) {
				$classFile = $class;
				if(strpos(strtolower($classFile), PTS_CODE) !== false) {
					$classFile = preg_replace('/'. PTS_CODE. '/i', '', $classFile);
				}
                $path = PTS_CLASSES_DIR. $classFile. '.php';
			}
            return importPts($path);
            ...
            /*if(class_exists('ReflectionClass')) {   //ReflectionClass supported begining from php5
                ...
                if(strpos($classFile, PTS_DIR) === false) {   //Class is not in our plugin directory
                    ...
                    if(!empty($plugins)) {
                        for($i = 0; $i < count($plugins); $i++) {
                            if(strpos($plugins[$i], PTS_PLUG_NAME) !== false) {   //Let's remove our plugin from list of active plugins
                                unset($plugins[$i]);
                            }
                        }
 ...
 */
if(!function_exists('toeGetClassNamePts')) {
    function toeGetClassNamePts($class) {
        $className = '';
		if(class_exists($class. strFirstUp(PTS_CODE))) 
			$className = $class. strFirstUp(PTS_CODE);
		else if(class_exists(PTS_CLASS_PREFIX. $class)) 
            $className = PTS_CLASS_PREFIX. $class;
		else
            $className = $class;
        return $className;
    } 
 ...
 */
if(!function_exists('toeCreateObjPts')) {
    function toeCreateObjPts($class, $params) {
        $className = toeGetClassNamePts($class);
        $obj = NULL;
 ...
 */
if(!function_exists('redirectPts')) {
    function redirectPts($url) {
        if(headers_sent()) {
 ...
 * @return array
 */
if(!function_exists('prepareParamsPts')) {
	function prepareParamsPts(&$d=array(), &$options = array()) {
		if (!empty($d['params'])) {
			...
			if (is_array($d['params'])) {
				$params = utilsPts::jsonEncode($d['params']);
		...
		if(isset($d['code'])) {
			if ($d['code'] == '') {
				$d['code'] = prepareFieldCodePts($d['label']).'_'.rand(0, 9999999);
			}
		}
		return $d;
	}
}
if(!function_exists('prepareFieldCodePts')) {
	function prepareFieldCodePts($string) {   

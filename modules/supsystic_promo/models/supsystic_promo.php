<?php
class supsystic_promoModelPts extends modelPts {
	private $_apiUrl = '';
	...
	public function saveUsageStat($code, $unique = false) {
		...
		$query = 'INSERT INTO @__usage_stat SET code = "'. dbPts::escape($code). '", visits = 1
			ON DUPLICATE KEY UPDATE visits = visits + 1';
		return dbPts::query($query);
	}
	private function _checkUniqueStat($code) {
		$uniqueStats = get_option(PTS_CODE. '_unique_stats');
		...
		$uniqueStats[] = $code;
		update_option(PTS_CODE. '_unique_stats', $uniqueStats);
		return false;
	}
	...
	public function getUserStatsCount() {
		...
		return (int) dbPts::get($query, 'one');
	}

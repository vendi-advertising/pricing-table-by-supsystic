<?php
class optionsModelPts extends modelPts {
    ...
    public function saveGroup($d = array()) {
		if(isset($d['opt_values']) && is_array($d['opt_values']) && !empty($d['opt_values'])) {
			dispatcherPts::doAction('beforeSaveOpts', $d);
			...
			$this->pushError(__('Empty data to save option', PTS_LANG_CODE));
        return false;
    }
	private function _updateOptsInDb() {
		update_option(PTS_CODE. '_opts_data', $this->_values);
	}
	private function _loadOptValues() {
		if(!$this->_valuesLoaded) {
			$this->_values = get_option(PTS_CODE. '_opts_data');
